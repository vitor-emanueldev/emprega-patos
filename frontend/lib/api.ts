...
export async function candidatarVaga(token: string, vagaId: string) {
  const resposta = await fetch(`${API_URL}/candidaturas`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  if (!resposta.ok) {
    throw new Error(dados.erro || "Erro ao se candidatar à vaga");
  }

  return dados;
}

export async function minhasCandidaturas(
  token: string
): Promise<Candidatura[]> {
  const resposta = await fetch(`${API_URL}/candidaturas/minhas`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export async function verificarCandidato(token: string) {
  const resposta = await fetch(`${API_URL}/candidatos/verificar`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  if (resposta.status === 404) {
    return null; // usuário não tem cadastro de candidato/currículo ainda
  }

  const dados = await resposta.json();

  if (!resposta.ok) {
    throw new Error(dados.erro || "Erro ao verificar candidato");
  }

  return dados; // ex: { temCurriculo: boolean }
}